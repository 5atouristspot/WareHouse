    ...
    try:
        ...
        if warehouse_type == 1:
            ...
            for rack_type in rack_type_l:
                sql_total = "select count(distinct(a.storage_bin)) from tasly_warehouse_storage_bin a left join tasly_warehouse_storage_info b on a.storage_bin = b.storage_bin where a.warehouse_type = 1 and a.rack_type = '{rack_type}' ;".format(rack_type=rack_type)
                db = MySQL(dbconfig)
                db.query(sql_total)
            ...
            sql_all_total = "select count(distinct(a.storage_bin)) from tasly_warehouse_storage_bin a left join tasly_warehouse_storage_info b on a.storage_bin = b.storage_bin where a.warehouse_type = 1 ;"
            db = MySQL(dbconfig)
