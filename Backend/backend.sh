...
Usage:
Options:
    --start|-s      start backend
    --stop|-p       stop backend
    --restart|-ps   restart backend
EOF
}
...
function modify_gunicorn_conf_bind()
{
    ...
    before_str="bind\s="
    after_str="bind = '$host_ip:3622'"
    sed -i "s#$before_str.*#$after_str#" ./config/gunicorn_conf.py

}

function run_gunicorn()
{
    nohup /data1/mycode/WareHouse/Backend/venv/bin/gunicorn -c config/gunicorn_conf.py run:app &
}

function sigterm_gunicorn()
{
    guni_pid_file="backend.pid"
